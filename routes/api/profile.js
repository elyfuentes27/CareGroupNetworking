...
// @access  Private
router.post('/update/experience', [auth, [
    check('title', 'Title is required').not().isEmpty(),
    check('company', 'Company is required').not().isEmpty(),
    check('from', 'From is required').not().isEmpty(),
]], async (req, res) => {
...
router.delete('/experience/:exp_id', auth, async (req, res) => {
    ...
    try {
        ...
        //Get remove index 
        const removeIndex = profile.experience.map(item => item.id).indexOf(req.params.exp_id); // eq.params.exp_id -> coming from the path match with the the map index item
...
router.delete('/education/:edu_id', auth, async (req, res) => {
    ...
    try {
        ...
        //Get remove index 
        const removeIndex = profile.education.map(item => item.id).indexOf(req.params.edu_id); // eq.params.exp_id -> coming from the path match with the the map index item
...
router.get('/github/:username', (req, res) => {
    ...
    try {
        const options = {
            uri: `https://api.github.com/users/${req.params.username}/repos?per_pages=5&sort=created:asc&client_id=${config.get('githubClientId')}&client_secret=${config.get('githubSecret')}`,
            method: 'GET',
            headers: {
                'user-agent': 'node.js'
            }
